...
function love.load()
    ...
    for i= 1,22 do
        if i ~= 22 then
            gFrames['bricks'][i] = gFrames['bricks'][i]
        else
            gFrames['bricks'][i] = gFrames['key'][1]
        end
    end
...
function displayFPS()
    ...
    -- x = 20
    -- for i= 1,22 do
    -- love.graphics.draw(gTextures['main'], gFrames['bricks'][i], x, y)
    --     x = x +40
...
-- love.graphics.draw(gTextures['main'], gFrames['paddles'][1], 50, 50)