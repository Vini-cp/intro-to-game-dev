...
function PlayState:update(dt)
    ...
    -- if self.reswap then
    --     self:swapPositions(self.highlightedTile,self.board, x, y, false)
    -- end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                local highlighted_old = self.highlightedTile
                
                self:swapPositions(self.highlightedTile,self.board, x, y, true)
            end
        end
    end
...
--[[
    Swap grid positions of tiles selected.
]]
function PlayState:swapPositions(highlightedTile,board, x, y, bol)
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for l, match2 in pairs(match) do
                self.score = self.score + match2.variety * 1
            end
        ...
        self.score_test = self.score_test + 1
    -- if no matches, we can continue playing
    else
        self.canInput = true
        
        if self.score_test == 0 then
            self:swapPositions(self.highlightedTile,self.board, x, y, false)
            gSounds['error']:play()
        end
        self.highlightedTile = nil
